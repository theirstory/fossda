...
export default function AskForm() {
  ...
  const [quotes, setQuotes] = useState<QuoteGroups>({ cited: [], uncited: [] });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !searchParams.get('q')) {
      const savedQuestion = localStorage.getItem('lastQuestion') || '';
      const savedAnswer = localStorage.getItem('lastAnswer') || '';
      const savedQuotes = JSON.parse(localStorage.getItem('lastQuotes') || '{"cited":[],"uncited":[]}');
      
      setQuestion(savedQuestion);
      setStreamedAnswer(savedAnswer);
